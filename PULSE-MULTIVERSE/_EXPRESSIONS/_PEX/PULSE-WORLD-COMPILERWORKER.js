...
import {createPulseWorldFightFlightResponseV40 as ACTNowFactory2} from "../../../PULSE-UNIVERSE/PULSE-GALAXY/PULSE-WORLD/PULSE-PHYSICS/X-PULSE-X/PulseWorldFightFlightResponse-v30.js"
import {PulsePowerAPIv32 as PulsePowerAPI2} from "../../../PULSE-UNIVERSE/PULSE-GALAXY/PULSE-WORLD/PULSE-PHYSICS/X-PULSE-X/PULSE-WORLD-POWER.js"
import {createPulseWorldChunker as ChunkerFactory2} from "../../../PULSE-UNIVERSE/PULSE-GALAXY/PULSE-WORLD/PULSE-PHYSICS/X-PULSE-X/PULSE-WORLD-CHUNKER.js"
import fs from "fs";
...
export function createPulseWorldCompilerWorker({
  PulseWorldCompile,
  ChunkerFactory=ChunkerFactory2,
  ACTNowFactory=ACTNowFactory2,
  PulsePowerAPI=PulsePowerAPI2,
  WorldBinaryCore,
  Router,
  Brain,
  Logger
  ...
  async function compileJob(rawJob, rawContext = {}) {
    ...
    const isImageJob =
      ...
      rawJob.every(f => /\.(png|jpg|jpeg|webp)$/i.test(f));
    ...
    if (!isImageJob) {
      ...
      return {
        ...
        laneId: job.laneId,
        envelopeId: job.envelopeId,
        error: "CompilerWorker only handles image chunking",
        presence
      };
    }
    ...
    // RAW IMAGE BUFFERS (chunkSurface)
    let chunkSurface = null;

    try {
      chunkSurface = rawJob.map(f => fs.readFileSync(f));
    } catch (e) {
      const msg = e.message || "Image load failed";
    ...
    let chunkMeta = null;
    try {
      const buffer = Buffer.from(JSON.stringify(chunkSurface), "utf8");
    ...
    const compileBytes = Buffer.byteLength(
      JSON.stringify(chunkSurface),
      "utf8"
    );

    // Binary surfaces (image mode)
    ...
    const response = {
      ...
      chunkSurface, // RAW IMAGE BUFFERS ONLY
